...
opp_run \
  ...
  '--*.gNodeB*.cellularNic.nrPhy.enableD2DCqiReporting=false' \
  ...
  '--*.car[0].app[1].destPort=4500' \
  '--*.car[0].app[1].sendInterval=0.002s' \
  '--*.car[0].app[1].startTime=5s' \
  '--*.car[0].app[1].stopTime=10s' \
  '--*.car[0].app[1].packetSizeBytes=1200' \
  '--*.car[1].app[1].typename="V2XNoiseReceiverApp"' \
  '--*.car[1].app[1].localPort=4500' \
  '--*.car[1].app[1].noiseLogFile="v2x_simu5g_noise_packets.csv"'
