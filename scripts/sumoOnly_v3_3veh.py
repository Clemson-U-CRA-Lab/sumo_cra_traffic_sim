...
if __name__=="__main__":
    ...
    front_s_t = np.array(leading_vehicle_speed_profile[:, 3])+40.0
    ...
    if live_plt:
        ...
        line_full_ref_v, = axs[1].plot(record_t, front_v_t, 'r:', label='Full Ref v')
    ...
    # Start simulation 
    traci.vehicle.setSpeed("nv0", 0.0)  # Set initial speed of leading vehicle to 0
    ...
    while sumo_sim_manager.step < END_TIME/SIM_STEP:
        ...
        if live_plt:
            ...
            dist2.append(veh_states_matrix[2][3])
            spd2.append(veh_states_matrix[2][2])
            ...
            line_spd2.set_data(times, spd2)
            line_ref_v.set_data(ref_times, cycle_vs)
            axs[1].relim(); axs[1].autoscale_view()

            ref_times_fromMPC = np.arange(MPC_REF_STAGES) * MPC_DT + sim_time
            line_ref_v2.set_data(ref_times_fromMPC, preds_v['nv1'])
            line_ref_s2.set_data(ref_times_fromMPC, preds_s['nv1'])
    ...
    plt.legend(['Ref', 'Leading Vehicle','nv1',  'mache'])
    
    plt.subplot(3,1,2)
    plt.plot(record_t, front_v_t, 'k--') 
    ...
    plt.legend(['Leading Vehicle','nv1', 'mache', 'mache_accCmd'])
