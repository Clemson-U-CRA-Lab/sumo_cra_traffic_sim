...
ATTACK_CTRL = Path("networksim/results/simu5g_actual_ue_attack_strong_after_patch/sumo_controller_log.csv")
ATTACK_TRACE = Path("networksim/results/simu5g_actual_ue_attack_strong_after_patch/network_degradation_trace.csv")

OUTDIR = Path("networksim/results/simu5g_actual_ue_attack_strong_after_patch")
...
def save_combined_controller_comparison(base_df, attack_df):
    ...
    axs[0].plot(t, attack_speed, label="Simu5G actual UE attack strong after patch")
    ...
    axs[1].plot(t, attack_gap, label="Simu5G actual UE attack strong after patch")
    axs[1].set_ylabel("Gap [m]")
    axs[1].grid(True)
    axs[1].legend()

    axs[2].plot(t, base_cmd, "--", label="Baseline")
    axs[2].plot(t, attack_cmd, label="Simu5G actual UE attack strong after patch")
    ...
    plt.tight_layout()
    plt.savefig(OUTDIR / "baseline_vs_actual_ue_attack_strong_after_patch_combined.png", dpi=160)
    plt.close()
...
def save_difference_plot(base_df, attack_df):
    ...
    axs[0].set_title("Simu5G actual UE attack strong after patch minus baseline")
    axs[0].grid(True)
    ...
    plt.tight_layout()
    plt.savefig(OUTDIR / "baseline_vs_actual_ue_attack_strong_after_patch_difference.png", dpi=160)
    plt.close()
...
def save_zoomed_difference_plot(base_df, attack_df, start=18.0, end=58.0):
    ...
    plt.tight_layout()
    plt.savefig(OUTDIR / "baseline_vs_actual_ue_attack_strong_after_patch_difference_zoom.png", dpi=160)
    plt.close()
...
def save_network_delay_plot(scheduled, delivered):
    ...
    plt.title("Simu5G actual UE attack strong after patch delay vs send time")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(OUTDIR / "actual_ue_attack_strong_after_patch_delay_vs_send_time.png", dpi=160)
    plt.close()
    ...
    if not merged.empty:
        ...
        plt.ylabel("Time [s]")
        plt.title("Timestamp decomposition: Simu5G actual UE attack strong after patch")
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
        plt.savefig(OUTDIR / "actual_ue_attack_strong_after_patch_timestamp_decomposition.png", dpi=160)
        plt.close()
...
def save_packet_summary(scheduled, delivered, dropped):
    ...
    plt.title("Simu5G actual UE attack strong after patch packet summary")
    plt.tight_layout()
    plt.savefig(OUTDIR / "actual_ue_attack_strong_after_patch_packet_summary.png", dpi=160)
    plt.close()
