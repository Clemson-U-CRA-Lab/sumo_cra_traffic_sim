#! /usr/bin/env python3
"""
HIL YAML-driven V2X sweep runner (SUMO only).

Vehicle MPC runs on another PC and is already connected.
For each run in scripts/x2v_automate.yaml:
1. Update the YAML attack params.
2. Run sumo2V_v5_nvN_v1.py.
3. Wait 5 seconds before the next run.
"""
...
def main():
    ...
    print(f"[sweep] HIL mode: SUMO only (vehicle MPC assumed running on other PC)")
    print(f"[sweep] USE_FALLBACK={USE_FALLBACK} (from x2v_constants.py)")
    print(f"[sweep] break between runs: {BREAK_SECONDS}s")
    ...
    for i, run in enumerate(runs):
        ...
        if i < len(runs) - 1:
            print(f"[sweep] waiting {BREAK_SECONDS}s before next run...")
