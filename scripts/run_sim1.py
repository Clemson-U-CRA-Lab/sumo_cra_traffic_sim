...
if __name__=="__main__":
    ...
    while sumo_sim_manager.step < 200:
        ...
        # Get vehicle states
        veh_states = [sumo_sim_manager.getVehicleStates(f"veh{i}") for i in range(4)]
        ...
        if USING_ONLINE_MPC:
            acc_traffic_step_t = traffic_online_MPC_control_step(*[val for state in veh_states for val in state[:3]],
                                                                sim_time, record_t, front_v_t, online_MPC_control)
